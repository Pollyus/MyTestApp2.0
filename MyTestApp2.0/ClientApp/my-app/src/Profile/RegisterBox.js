...
function RegisterBox(props) {
    ...
    const Register = (e) => {
        e.preventDefault();
        const value = { "email": login, "password": password, "passwordConfirm" : verPassword };
        axios.post(
            'https://localhost:32770/api/Account/Register', value, { withCredentials: true }
        )
            .then((response) => { 
                ...
                <Box sx={{width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', m: 'auto'}}>
                    ...
                    <Box sx={{padding: 1, display: 'flex', alignItems: 'center', flexDirection: 'row'}}> 
                        ...
                        <TextField
                            ...
                            type="password"
                            autoComplete="current-password"
                            value={password} onChange={handleVerPasswordChange}
                        />
                    </Box>
                </Box>
